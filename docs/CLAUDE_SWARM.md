...
| `lms-expert` | LMS/learning-science advisory: feature-completeness audits vs the market, learning mechanics (assessment/mastery, paths, certs/recert), xAPI vocabulary + standards judgment (SCORM/cmi5/LTI/LRS) | read-only + web | ADVISORY — reports findings/specs; owners implement |
| `ui-ux` | UI/UX + accessibility advisory: field-first ergonomics, WCAG 2.2 AA, offline-state UX, the five screen states, token-system compliance, console IA | read-only + web | ADVISORY — design reviews and specs against real code; owners implement |
| `ehs-expert` | EHS compliance advisory: regs → product requirements (training matrix, cert expiry/recert, OJT sign-off, toolbox talks, designations), auditor-acceptable evidence, data-model expressiveness | read-only + web | ADVISORY — the compliance conscience; prioritizes by regulatory exposure |
| `ios-developer` | iOS platform posture of `apps/mobile`: Expo iOS config-as-code, Keychain session storage, APNs, universal links/QR, privacy manifests + App Store readiness, VoiceOver/Dynamic Type, EAS-as-code | file edit, bash, web | Guest under `mobile` conventions; offline store stays with `offline-sync`; parity with `android-developer` |
| `android-developer` | Android platform posture of `apps/mobile`: Expo Android config-as-code, Doze-safe sync scheduling, FCM, Keystore storage, Play readiness (target API, Data safety), rugged/MDM/kiosk + shared-device patterns, TalkBack | file edit, bash, web | Guest under `mobile` conventions; offline store stays with `offline-sync`; parity with `ios-developer` |
