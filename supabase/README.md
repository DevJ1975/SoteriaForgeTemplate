...
| **pending migration** | `12_progress_verbs_and_advisor_fixes` — authored, **not yet applied** (`supabase db push`): progress counts `completed`+`passed` verbs exactly, RLS init-plan perf, FK covering indexes, revoke `rls_auto_enable` execute |
