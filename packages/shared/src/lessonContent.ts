 ...
 * (the column defaults to `'{}'::jsonb` server-side), so this module defines the
 * platform's minimal content contract and parses it DEFENSIVELY: absent, empty,
 * or malformed content degrades to "no renderable content" (the player falls
 * back to the lesson description + mark-complete flow) instead of ever crashing
 * the player. The documented shape, which future console authoring should write:
 *
 * ```jsonc
 * {
